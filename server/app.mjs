/**
 * Aplicación Express (rutas de la API de ofertas).
 *
 * Se separa de `server/index.mjs` para poder, según el entorno:
 *  - Correr localmente con `node server/index.mjs` (hace `app.listen`).
 *  - Exportarse como función serverless en Vercel (`api/index.js`), que
 *    reutiliza esta misma `app` sin `listen`.
 *
 * Endpoints:
